...
// ===== DB HELPERS =====
function getDB(k) {
  if (window.PSDataStore && typeof window.PSDataStore.getDB === 'function') {
    return window.PSDataStore.getDB(k);
  }
  try { return JSON.parse(localStorage.getItem('ps_' + k) || '[]'); } catch { return []; }
}
function setDB(k, v) {
  if (window.PSDataStore && typeof window.PSDataStore.setDB === 'function') {
    window.PSDataStore.setDB(k, v);
    return;
  }
  localStorage.setItem('ps_' + k, JSON.stringify(v));
}
...
document.addEventListener('DOMContentLoaded', () => {
  if (window.PSDataStore && typeof window.PSDataStore.init === 'function') {
    window.PSDataStore.init();
  }
  seedData();
  updateCartCount();
});
